...
function Accordion() {
  const [openAlgebra, setOpenAlgebra] = useState(false);
  const navigation = useNavigation<HomeScreenNavigationProp>();

  const lessons = [
    ['Noções da função', 'Introdução da Função Afim'],
  ];

  return (
    <View style={styles.accordionContainer}>
      <TouchableOpacity style={styles.accordionHeader} onPress={() => setOpenAlgebra(!openAlgebra)}>
        <Text style={styles.accordionTitle}>Álgebra Elementar</Text>
        <MaterialIcons name={openAlgebra ? "keyboard-arrow-up" : "keyboard-arrow-down"} size={40} color="#fff" />
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  accordionHeader: {
    ...
    backgroundColor: '#707DCB', paddingVertical: 20, paddingHorizontal: 25, borderRadius: 25, marginBottom: 10,
  },
  accordionTitle: { fontSize: 28, fontWeight: 'bold', color: '#fff' },
  accordionContent: { padding: 20, backgroundColor: '#4d547cff', borderRadius: 20, marginBottom: 15 },
  row: { flexDirection: 'row', justifyContent: 'space-around', marginBottom: 15 },
  lessonBox: {
    flex: 1, marginHorizontal: 10, paddingVertical: 25, borderRadius: 20, backgroundColor: '#BDC4EE',
    alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOpacity: 0.15,
    shadowRadius: 10, shadowOffset: { width: 0, height: 5 }, elevation: 4,
  },
  lessonText: { fontSize: 22, fontWeight: 'bold', color: '#242948', textAlign: 'center' },
});

...
const Lobby: React.FC<LobbyProps> = ({ route }) => {
  ...
  const [timer, setTimer] = useState<null | number>(null); // Controla o valor do temporizador
  ...
  const checkAllReady = () => {
    // Verifica se todos os jogadores têm 'is_ready' como true
    return players.every(player => player.is_ready);
  };
  
  // Exemplo de uso
  useEffect(() => {
    if (checkAllReady() && !isGameActive) {
      setTimer(5);
  ...
  useEffect(() => {
    ...
    const countdown = setInterval(() => {
      setTimer(prev => {
        ...
        return prev !== null ? prev - 1 : null; // Verifica se prev não é null
      });
    }, 1000);
...
const startGame = async () => {
  ...
  return (
    <View style={styles.container}>
      ...
      {timer !== null && <Text style={styles.timer}>Iniciando em: {timer} segundos</Text>}
